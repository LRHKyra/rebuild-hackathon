...
export async function POST(request: Request) {
  ...
  // Speak the dedicated spoken register (already concise + markdown-free).
  const spokenText =
    latestAnswer?.canSpeak && latestAnswer.spokenAnswer
      ? latestAnswer.spokenAnswer
      : UNSUPPORTED_SPOKEN;
...
async function answerForSummon(params: {
  ...
  if (retrieved.length === 0) {
    const refusal: AnswerCard = {
      ...
      spokenAnswer: UNSUPPORTED_SPOKEN,
      sourceCardIds: [],
      confidence: "low",
      canSpeak: false,
      createdAt,
  ...
  const answer: AnswerCard = {
    ...
    spokenAnswer: generated.spokenAnswer,
